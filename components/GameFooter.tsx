...
const GameFooter: React.FC<GameFooterProps> = ({
  ...
  if (layout === 'portrait') {
    ...
    return (
      <footer className={`w-full px-2 pt-2 pb-safe-bottom bg-black/30 backdrop-blur-sm flex-shrink-0`}>
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center">
          ...
          <div className="w-1/4 flex justify-end items-center relative">
              {completedSetsContent}
              <button ref={menuButtonRef} onClick={() => setIsMenuOpen(prev => !prev)} className={portraitButtonClasses} disabled={isDealing}>
              ...
              {isMenuOpen && (
                <div ref={menuRef} className="absolute bottom-full right-0 mb-2 w-48 bg-white/90 backdrop-blur-md rounded-lg shadow-xl animate-fade-in-up origin-bottom-right">
                    <div className="p-2 space-y-1">
                        ...
                        {onLog && <button onClick={onLog} className="w-full text-left p-3 rounded-lg text-base font-semibold transition-colors bg-gray-100 hover:bg-green-100 text-gray-700">Game Log</button>}
                    </div>
                </div>
              )}
          </div>
  ...
  return (
    <footer className={`w-full px-4 pt-2 pb-4 bg-black/20 flex-shrink-0`}>
      <div className="max-w-7xl mx-auto w-full flex justify-center items-center gap-4">
        ...
        <button onClick={onRules} disabled={isDealing} className={landscapeButtonClasses}>
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>
          <span>Rules</span>
        </button>
        {children}
        {onLog && (
            <button onClick={onLog} disabled={isDealing} className={landscapeButtonClasses}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                <span>Log</span>
            </button>
        )}
        <button onClick={onSettings} disabled={isDealing} className={landscapeIconButtonClasses} aria-label="Settings">
